...
public static async Task<object> Run(HttpRequestMessage req, TraceWriter log)
{
    ...
    var warningThreshold = data?.WarningThreshold ?? 10000;
    var criticalThreshold = data?.CriticalThreshold ?? 5000;
    ...
    {
        message +=  $" - {result.Computer} : [{(result.Min/100):N0} free]";
    ...
    message = $"Infra - Memory {(critical ? "Critical" : "Warning")}" + message;
